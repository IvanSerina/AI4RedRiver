...
A dashboard developed for the AI4RedRiver project using Streamlit for water resource management and plan generation.

# Installation

 - Create a virtual environment
 	- `cd dashboard_redriver`
  	- `python3 -m venv venv`
    - `source venv/bin/activate` 
 - Install java `sudo apt install openjdk-21-jdk`
 - Install requirements `pip install -r requirements.txt`
 - Create default admin user: run the python command `python3 admin_user_generator.py`
	 - Now there is an admin user with username: *admin* and password: *Redriver2025@*
	 - The password can be modified from the dashboard using the "Admin" page.

# Usage

Run the command `bash start_dashboard.sh` from a new shell, starting in the initial directory.

Now it's possible to use the dashboard. Below is a detailed explanation of the dashboard pages and features.
...
#   How to transfer project code
In order to transfer the project code to an entity that will manage the code you need to remove some sensitive data:

 - Delete all saved sessions from the folder `saved_sessions`
 - Delete all actual users, that is deleting all content or delete the file `users.yaml`
 - (optional) delete the historical data csv from the folder `historical_data` and remember to the final user that is necessary to upload an historical_data.csv file inside the historical_data project folder.
